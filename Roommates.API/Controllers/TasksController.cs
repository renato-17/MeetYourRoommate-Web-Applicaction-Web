...
namespace Roommates.API.Controllers
{
    ...
    public class TasksController : ControllerBase
    {
        ...
        [SwaggerOperation(
         Summary = "Delete task",
         Description = "Delete an specific task",
         OperationId = "DeleteTask",
         Tags = new[] { "tasks" }
         )]
        [HttpDelete("{id}")]
        public async Task<IActionResult> DeleteAsync(int id, int teamId)
        {
            var result = await _taskService.DeleteAsync(id, teamId);

            if (!result.Success)
                return BadRequest(result.Message);

            var studyCenterResource = _mapper.Map<Domain.Models.Task, TaskResource>(result.Resource);
