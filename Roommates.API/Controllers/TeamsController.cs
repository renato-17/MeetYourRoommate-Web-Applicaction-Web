...
namespace Roommates.API.Controllers
{
    ...
    public class TeamsController : ControllerBase
    {
        ...
        [SwaggerOperation(
          Summary = "Update Team",
          Description = "Update an specific Team",
          OperationId = "UpdateTeam",
          Tags = new[] { "teams" }
          )]
        [HttpPut("{id}")]
        public async Task<IActionResult> PutAsync([FromBody] SaveTeamResource resource, int id)
        {
            var team = _mapper.Map<SaveTeamResource, Team>(resource);
            var result = await _teamService.UpdateAsync(team, id);
