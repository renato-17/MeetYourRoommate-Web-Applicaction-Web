...
namespace Roommates.API.Controllers
{
    ...
    public class StudyCentersController: ControllerBase
    {
        ...
        [SwaggerOperation(
          ...
          Description = "Update an specific Study center",
          OperationId = "UpdateStudyCenter",
          Tags = new[] { "study_centers" }
          )]
        [HttpPut("{id}")]
        public async Task<IActionResult> PutAsync([FromBody] SaveStudyCenterResource resource, int id)
        {
            var studyCenter = _mapper.Map<SaveStudyCenterResource, StudyCenter>(resource);
            var result = await _studyCenterService.UpdateAsync(id, studyCenter);
